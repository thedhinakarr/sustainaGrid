...
export default function ProducerSourceProfile() {
...
async function SubClick(e){
  e.preventDefault();
  navigate(`/producerCheckoutPage/${sdata._id}`)
}
...
function locationAnalysisClick(e){
  e.preventDefault();
  navigate(`/producerLocationAnalyser/${sdata._id}`);
}
    ...
    <div>
      <div className="flex flex-row h-screen text-white">
        ...
        <div className="lex flex-nowrap flex-col h-screen w-3/5 py-9 px-5 border-susZGreen ">
          ...
          <button onClick={SubClick} className="font-semibold  hover:bg-susZGreen w-40 h-10 py-1 mr-3 text-center border">Buy Source</button>
          <div className="font-semibold mb-2  text-lg p-3  rounded-lg items-center  border-susZGreen border">Source Cost: <span className="text-susZGreen">Rs.{sdata.sourceCost}</span></div>
